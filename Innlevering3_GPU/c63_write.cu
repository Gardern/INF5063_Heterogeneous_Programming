...
static void write_SOI(struct c63_common_cpu *cm)
{
...
static void write_DQT(struct c63_common_cpu *cm)
{
...
static void write_SOF0(struct c63_common_cpu *cm)
{
  ...
  put_byte(cm->e_ctx.fp, cm->keyframe);
}

static void write_DHT_HTS(struct c63_common_cpu *cm, uint8_t id, uint8_t *numlength,
    uint8_t* data)
{
...
/* Define Huffman Table (DHT) marker, the payload is the Huffman table
   specifiation. */
static void write_DHT(struct c63_common_cpu *cm)
{
...
static void write_SOS(struct c63_common_cpu *cm)
{
...
static void write_EOI(struct c63_common_cpu *cm)
{
  put_byte(cm->e_ctx.fp, JPEG_DEF_MARKER);
...
static void write_block(struct c63_common_cpu *cm, int16_t *in_data, uint32_t width,
    ...
    int32_t cc, int channel)
{
  ...
  struct macroblock *mb =
    &cm->mbs[channel][voffset/8 * cm->padw[channel]/8 + uoffset/8];
...
static void write_interleaved_data_MCU(struct c63_common_cpu *cm, int16_t *dct,
...
static void write_interleaved_data(struct c63_common_cpu *cm)
{
  ...
  for(v = 0; v < vblocks; ++v)
	{
		for(u = 0; u < ublocks; ++u)
		{
			write_interleaved_data_MCU(cm, cm->residuals->Ydct, cm->ypw, cm->yph, YX, YY, u, v, &prev_DC[0], yhtbl, 0);
			write_interleaved_data_MCU(cm, cm->residuals->Udct, cm->upw, cm->uph, UX, UY, u, v, &prev_DC[1], uhtbl, 1);
			write_interleaved_data_MCU(cm, cm->residuals->Vdct, cm->vpw, cm->vph, VX, VY, u, v, &prev_DC[2], vhtbl, 2);
		}
	}
