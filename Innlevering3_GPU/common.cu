...
struct frame* create_frame(struct c63_common_cpu *cm, yuv_t *image)
{
  ...
  f->mbs[0] = (macroblock*) malloc(cm->mb_rows * cm->mb_cols * sizeof(struct macroblock));
  f->mbs[1] = (macroblock*) malloc(cm->mb_rows/2 * cm->mb_cols/2 * sizeof(struct macroblock));
  f->mbs[2] = (macroblock*) malloc(cm->mb_rows/2 * cm->mb_cols/2 * sizeof(struct macroblock));
