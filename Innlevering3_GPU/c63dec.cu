...
static void read_block(struct c63_common_cpu *cm, int16_t *out_data, uint32_t width,
...
static void read_interleaved_data_MCU(struct c63_common_cpu *cm, int16_t *dct,
...
void read_interleaved_data(struct c63_common_cpu *cm)
{
  int u,v;
...
void parse_dqt(struct c63_common_cpu *cm)
{
  int i;
  uint16_t size;
...
// Start of scan
void parse_sos(struct c63_common_cpu *cm)
{
  uint16_t size;
  size = (get_byte(cm->e_ctx.fp) << 8) | get_byte(cm->e_ctx.fp);
  ...
  buf.reserve(size);
  read_bytes(cm->e_ctx.fp, (void*)&buf, size-2);
#endif
}

// Baseline DCT
void parse_sof0(struct c63_common_cpu *cm)
{
  uint16_t size;
  size = (get_byte(cm->e_ctx.fp) << 8) | get_byte(cm->e_ctx.fp);
...
void parse_dht(struct c63_common_cpu *cm)
{
  uint16_t size;
  size = (get_byte(cm->e_ctx.fp) << 8) | get_byte(cm->e_ctx.fp);
...
int parse_c63_frame(struct c63_common_cpu *cm)
{
  // SOI
...
void decode_c63_frame(struct c63_common_cpu *cm, FILE *fout)
{
  /* Motion Compensation */
...
int main(int argc, char **argv)
{
  ...
  struct c63_common_cpu *cm = (c63_common_cpu *)calloc(1, sizeof(*cm));
  cm->e_ctx.fp = fin;
