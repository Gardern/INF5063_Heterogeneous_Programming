...
dct_parm_data spe_dct_data __attribute__ ((aligned (16)));
...
static void dct_1d(float *in_data, float *out_data)
{
	...
	for (i = 0; i < 8; ++i)
	{
		...
		vector float table1 = (vector float) {dctlookup[0][i], dctlookup[1][i], dctlookup[2][i], dctlookup[3][i]};

		tempResult = spu_madd(in_data1, table1, tempResult);

		vector float in_data2 = (vector float) {in_data[4], in_data[5], in_data[6], in_data[7]};
		vector float table2 = (vector float ) {dctlookup[4][i], dctlookup[5][i], dctlookup[6][i], dctlookup[7][i]};
...
static void quantize_block(float *in_data, float *out_data, uint8_t *quant_tbl)
{
	...
	for (zigzag = 0; zigzag < 64;)
	{
		...
		vector float result1 = spu_mul(dct, zero_p_twof);
		result1 = divf4(result1, quant);

		out_data[zigzag] = round(result1[0]);
		out_data[zigzag+1] = round(result1[1]);
		out_data[zigzag+2] = round(result1[2]);
		out_data[zigzag+3] = round(result1[3]);
		...
		int16_t *out_data, uint8_t *quantization)
{
  ...
  for(x = 0; x < w/8*64; x += 64)
  {
    ...
    for (i = 0; i < 64; ++i)
    {;
	  block[i] = ((int16_t)in_data[x+i] - prediction[x+i]);
    }
    ...
    /* Store MBs linear in memory, i.e. the 64 coefficients are stored
       ...
       functions. */
    dct_quant_block_8x8(block, out_data+x, quantization); //out_data+(x*8)
  }
}
...
int main(unsigned long long spe, unsigned long long argp, unsigned long long envp)
{
	...
	spu_mfcdma32(&spe_dct_data, argp, sizeof(spe_dct_data), tag, MFC_GET_CMD);
	spu_writech(MFC_WrTagMask, 1 << tag);
	spu_mfcstat(MFC_TAG_UPDATE_ALL);

	spu_mfcdma32((void*) quantization, (unsigned int)spe_dct_data.quantization, 64 * sizeof(uint8_t), tag, MFC_GET_CMD);

	int range = spe_dct_data.range;

	int i;
  	for (i = spe_dct_data.y; i < range; ++i)
  	{
		spu_mfcdma32((void*) in_data, (unsigned int)spe_dct_data.in_data+i*((spe_dct_data.width/8)*64), ((spe_dct_data.width/8))*64 * sizeof(uint8_t), tag, MFC_GET_CMD);
		spu_mfcdma32((void*) prediction, (unsigned int)spe_dct_data.prediction+i*((spe_dct_data.width/8)*64), ((spe_dct_data.width/8))*64 * sizeof(uint8_t), tag, MFC_GET_CMD);
		spu_writech(MFC_WrTagMask, 1 << tag);
		spu_mfcstat(MFC_TAG_UPDATE_ALL);

    	dct_quantize_row(in_data, prediction, spe_dct_data.width, spe_dct_data.height,
    			out_data, quantization);

		spu_mfcdma32((void*) out_data, (unsigned int)spe_dct_data.out_data+(i*((spe_dct_data.width/8)*64))*sizeof(int16_t), ((spe_dct_data.width/8))*64 * sizeof(int16_t), tag, MFC_PUT_CMD);
		spu_writech(MFC_WrTagMask, 1 << tag);
		spu_mfcstat(MFC_TAG_UPDATE_ALL);
  	}
}
